...
impl PeerActor {
    ...
    fn receive_message(
        ...
        ctx.spawn(wrap_future(handling_future.in_current_span()).map(
    ...
    fn handle_msg_ready(
        ...
        match peer_msg {
            ...
            PeerMessage::PorMessage(message) => {
                if let Some(por_handler) = &self.network_state.por_handler {
                    por_handler.handle_message(&conn.peer_info.id, message);
                }
            }
